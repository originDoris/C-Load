package com.cl.framework.core.utils.jackson;
 ...
 * @author xhz
 */
public class DataTableModule extends SimpleModule {
    ...
    public DataTableModule() {
        ...
        this.addSerializer(TIMESTAMP.class, new OracleTimestampSerializer());
            ...
            public ValueInstantiator findValueInstantiator(final DeserializationConfig config, final BeanDescription beanDesc, final ValueInstantiator defaultInstantiator) {
                ...
                if (ZoneId.class.isAssignableFrom(raw) && defaultInstantiator instanceof StdValueInstantiator) {
                    ...
                    if (!inst.canCreateFromString()) {
                        final AnnotatedMethod factory = DataTableModule.this.findFactory(ac, String.class);
                        if (factory != null) {
    ...
    protected AnnotatedMethod findFactory(final AnnotatedClass cls, final Class... argTypes) {
        final int argCount = argTypes.length;
        final Iterator<AnnotatedMethod> var5 = cls.getFactoryMethods().iterator();
